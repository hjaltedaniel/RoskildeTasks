<template>
	<div v-if="messages.length != 0" class="d-flex flex-column message-view">
		<div class="single-message d-flex align-items-center" v-bind:class="{ adminMessage: message.isFromAdmin }" v-for="message in olderMessages" :key="message.Date">
		...
		<div class="message-view__breaker d-flex justify-content-center" v-if="yesterdaysMessages != ''">
			<span class="breaker-line"></span>
			<span class="breaker-content">Yesterday</span>
		</div>
		<div class="single-message d-flex align-items-center" v-bind:class="{ adminMessage: message.isFromAdmin }" v-for="message in yesterdaysMessages" :key="message.Date">
			<span class="single-message__time">
				{{getTimeFromDate(message.Date)}}
			</span>
			<div class="single-message__content">
		...
		<div class="single-message d-flex align-items-center" v-bind:class="{ adminMessage: message.isFromAdmin }" v-for="message in todaysMessages" :key="message.Date">
			<span class="single-message__time">
				{{getTimeFromDate(message.Date)}}
			</span>
			<div class="single-message__content">
